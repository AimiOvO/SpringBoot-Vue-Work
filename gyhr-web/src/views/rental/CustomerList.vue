    ...
    <el-main height="">
        ...
        <el-form :model="parms" ref="searchForm" label-width="80px" :inline="true" size="small">
            ...
            <el-form-item>
                ...
                <el-button v-if="hasPerm('wy:customer:add')" @click="addBtn" type="primary" icon="el-icon-plus">新增</el-button>
            </el-form-item>
        </el-form>
        <!--
            楼栋列表
        ...
        <el-table :data="tableList" empty-text="暂无数据" :height="tableHeight" size="small" border stripe>
            ...
            <el-table-column align="center" width="300" label="操作">
                <template slot-scope="scope">
                    <el-button v-if="hasPerm('wy:customer:edit')" type="primary" icon="el-icon-edit" size="small" @click="editBtn(scope.row)">编辑</el-button>
                    <el-button v-if="hasPerm('wy:customer:delete')" type="danger" icon="el-icon-delete" size="small" @click="deleteBtn(scope.row)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!--
            ...
            <div slot="content">
                ...
                <el-form :model="addModel" ref="addForm" :rules="rules" label-width="100px" :inline="true" size="small"
                    ...
                    <el-form-item prop="cname" label="姓名">
                        <el-input v-model="addModel.cname"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item prop="cphone" label="电话">
                        <el-input v-model="addModel.cphone"></el-input>
                    </el-form-item>
...
<script>
import { getCustomerListApi, addCustomerApi, editCustomerApi, deleteCustomerApi } from "@/api/rental/customer"
...
export default {
    ...
    data() {
        return {
            //表单验证规则
            rules: {
                cname: [{ trigger: "change", required: true, message: "请填写姓名" }],
                cphone: [
                ...
                username: [{ trigger: "change", required: false, message: "请填写登录名" }],
                password: [{ trigger: "change", required: false, message: "请填写登录密码" }],
                enabled: [{ trigger: "change", required: true, message: "请选择用户状态" }],
            },
            //表单绑定数据源
            addModel: {
                formtype: "",
    ...
    methods: {
        ...
        async changeUsed(row) {
            ...
            let res = await editCustomerApi(parm);
            if (res && res.code == 200) {
                this.getListData();
        ...
        // 删除按钮事件
        async deleteBtn(row) {
            let confrim = await this.$myconfirm("确定删除该租户吗？");
            if (confrim) {
                let res = await deleteCustomerApi({ customerId: row.customerId });
                if (res && res.code == 200) {
                    // 删除成功刷新列表
                    this.getListData();
                    this.$message.success(res.msg);
            ...
            this.$refs.addForm.validate(async (valid) => {
                if (valid) {
                    ...
                    if (this.addModel.formtype == "0") {
                        // 新增
                        res = await addCustomerApi(this.addModel);
                    } else if (this.addModel.formtype == "1") {
                        // 编辑
                        res = await editCustomerApi(this.addModel);
                    }
                    if (res && res.code == 200) {
                        // 请求成功刷新列表数据
                        this.getListData();
        ...
        async getListData() {
            let res = await getCustomerListApi(this.parms);
            console.log(res);
            if (res && res.code == 200) {
