    ...
    <el-main height="">
        ...
        <el-form :model="parms" ref="searchForm" label-width="80px" :inline="true" size="small">
            <el-form-item label="租单编号">
                <el-input v-model="parms.rentalparkNum"></el-input>
            </el-form-item>
            ...
            <el-form-item>
                ...
                <el-button v-if="hasPerm('wy:rentalpark:add')" @click="addBtn" type="primary" icon="el-icon-plus">新增</el-button>
            </el-form-item>
        </el-form>
        <!--
            楼栋列表
        ...
        <el-table :data="tableList" empty-text="暂无数据" :height="tableHeight" size="small" border stripe>
            <el-table-column prop="rentalparkNum" label="租单编号"></el-table-column>
            <el-table-column align="center" width="100" prop="cname" label="租户姓名"></el-table-column>
            <el-table-column align="center" width="100" prop="parkName" label="车位名称"></el-table-column>
            ...
            <el-table-column align="center" width="100" prop="status" label="租单状态">
                <template slot-scope="scope">
                    <el-tag type="success" size="normal" v-if="scope.row.status == '0'">在期</el-tag>
                    <el-tag type="danger" size="normal" v-if="scope.row.status == '1'">结束</el-tag>
                </template>
            </el-table-column>
            <el-table-column align="center" width="200" label="操作">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.status == '0' && hasPerm('wy:rentalpark:return')" type="warning" icon="el-icon-circle-close" size="small"
                        @click="leavePark(scope.row)">退车位</el-button>
                    <el-button v-if="scope.row.status == '1' && hasPerm('wy:rentalpark:edit')" type="primary" icon="el-icon-edit" size="small"
                        @click="editBtn(scope.row)">编辑</el-button>
                    <el-button  v-if="hasPerm('wy:rentalpark:delete')" type="danger" icon="el-icon-delete" size="small" @click="deleteBtn(scope.row)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!--
            ...
            <div slot="content">
                ...
                <el-form :model="addModel" ref="addForm" :rules="rules" label-width="100px" :inline="true" size="small"
                    ...
                    <el-form-item prop="parkName" label="所属车位">
                        ...
                        <el-button type="primary" size="small" @click="selectPark()">选择车位</el-button>
                    </el-form-item>
...
<script>
import { getParkListApi, addRentalParkApi, editRentalParkApi, deleteRentalParkApi, returnParkApi, getRentalParkListApi, getCustomerListApi } from '@/api/rental/rentalpark'
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                status: [{ trigger: "change", required: true, message: "请租单状态" }],
            },
            addModel: {
                formtype: "",
                customerParkId: "",
            ...
            //分配租户列表高度
            customerTableHeight: window.innerHeight - 940,
            //分配租户参数
            assignCustomerParm: {
                customerId: "",
                cname: ""
    ...
    methods: {
         ...
         */
        selectPark() {
            this.parkParms.cname = "";
            this.parkParms.cphone = "";
            this.assignParkParm.parkId = this.addModel.parkId;
        ...
        //退车位
        async leavePark(row) {
            let confrim = await this.$myconfirm("确定退停车位吗？");
            if (confrim) {
                let parm = {
                    customerParkId: row.customerParkId,
                    parkId: row.parkId,
        ...
        async deleteBtn(row) {
            ...
            if (confrim) {
                let res = await deleteRentalParkApi({ customerParkId: row.customerParkId });
                if (res && res.code == 200) {
                    // 删除成功刷新列表
                    this.getListData();
                    this.$message.success(res.msg);
            ...
            this.$refs.addForm.validate(async (valid) => {
                if (valid) {
                    ...
                    if (this.addModel.formtype == "0") {
                        // 新增
                        res = await addRentalParkApi(this.addModel);
                    } else if (this.addModel.formtype == "1") {
                        // 编辑
                        res = await editRentalParkApi(this.addModel);
                    }
                    if (res && res.code == 200) {
                        // 请求成功刷新列表数据
                        this.getListData();
        ...
        async getListData() {
            let res = await getRentalParkListApi(this.parms);
            console.log(res);
            if (res && res.code == 200) {
