    ...
    <el-main height="">
        ...
        <el-form :model="parms" ref="searchForm" label-width="80px" :inline="true" size="small">
            ...
            <el-form-item>
                ...
                <el-button v-if="hasPerm('wy:waterfee:add')" @click="addBtn" type="primary"
        ...
        <el-table :data="tableList" empty-text="暂无数据" :height="tableHeight" size="small" border stripe>
            ...
            <el-table-column align="center" prop="waterNum" label="表显（方）"></el-table-column>
            ...
            <el-table-column align="center" width="200" label="操作">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.payStatus == '0' && hasPerm('wy:waterfee:pay')" type="success"
                        icon="el-icon-wallet" size="small" @click="payBtn(scope.row)">缴费</el-button>
                    <el-button v-if="hasPerm('wy:waterfee:edit') && scope.row.payStatus == '0'" type="primary"
                        icon="el-icon-edit" size="small" @click="editBtn(scope.row)">编辑</el-button>
                    <el-button v-if="hasPerm('wy:waterfee:delete')" type="danger" icon="el-icon-delete" size="small"
        ...
        <sys-dialog :title="dialog.title" :visible="dialog.visible" :width="dialog.width" :height="dialog.height"
            ...
            <div slot="content">
                ...
                <el-form :model="addModel" ref="addForm" :rules="rules" label-width="100px" :inline="true" size="small"
                    ...
                    <el-form-item prop="waterNum" label="表显" size="small">
                        <el-input  @change="NumToMoney" v-model="addModel.waterNum"></el-input>
                    </el-form-item>
...
import { getWaterFeeListApi, getOnHomeList, addWaterFeeApi, editWaterFeeApi, deleteWaterFeeApi, payWaterFeeApi } from "@/api/fee/water"
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                waterNum: [{ trigger: "change", required: true, message: "请填写表显" }],
    ...
    methods: {
        NumToMoney() {
            this.addModel.payMoney = this.addModel.waterNum;
        },
        /**
         * 选择房屋
         */
        ...
        async payBtn(row) {
            ...
            if (confrim) {
                let parm = {
                    waterfeeId: row.waterfeeId,
                    payStatus: '1'
                }
                let res = await payWaterFeeApi(parm);
                if (res && res.code == 200) {
                    // 删除成功刷新列表
                    this.getListData();
        ...
        async deleteBtn(row) {
            ...
            if (confrim) {
                let res = await deleteWaterFeeApi({ waterfeeId: row.waterfeeId });
                if (res && res.code == 200) {
                    // 删除成功刷新列表
                    this.getListData();
                    this.$message.success(res.msg);
            ...
            this.$refs.addForm.validate(async (valid) => {
                if (valid) {
                    ...
                    if (this.addModel.formtype == "0") {
                        // 新增
                        res = await addWaterFeeApi(this.addModel);
                    } else if (this.addModel.formtype == "1") {
                        // 编辑
                        res = await editWaterFeeApi(this.addModel);
                    }
                    if (res && res.code == 200) {
                        // 请求成功刷新列表数据
                        this.getListData();
        ...
        async getListData() {
            let res = await getWaterFeeListApi(this.parms);
            console.log(res);
            if (res && res.code == 200) {
