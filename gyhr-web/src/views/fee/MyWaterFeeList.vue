<template>
    <el-main height="">
        ...
        <el-table :data="tableList" empty-text="暂无数据" :height="tableHeight" size="small" border stripe>
            ...
            <el-table-column align="center" prop="waterNum" label="表显"></el-table-column>
            ...
            <el-table-column align="center" width="100" label="操作">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.payStatus == '0' && hasPerm('wy:waterfee:pay')" type="success"
                        icon="el-icon-wallet" size="small" @click="alipay(scope.row)">缴费</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!--
...
<script>
import { getWaterFeeListApi, payWaterFeeApi } from "@/api/fee/water"
...
export default {
    ...
    methods: {
        ...
        async payBtn(row) {
            let confrim = await this.$myconfirm("确定缴费吗？");
            if (confrim) {
                let res = await payWaterFeeApi({ waterfeeId: row.waterfeeId });
                if (res && res.code == 200) {
                    // 删除成功刷新列表
                    this.getListData();
