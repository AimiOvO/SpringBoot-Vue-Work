    ...
    <el-main height="">
        ...
        <el-form :model="parms" ref="searchForm" label-width="80px" :inline="true" size="small">
            ...
            <el-form-item>
                ...
                <el-button v-if="hasPerm('wy:rentalfee:add')" @click="addBtn" type="primary"
        ...
        <el-table :data="tableList" empty-text="暂无数据" :height="tableHeight" size="small" border stripe>
            ...
            <el-table-column align="center" width="200" label="操作">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.payStatus == '0' && hasPerm('wy:rentalfee:pay')" type="success"
                        icon="el-icon-wallet" size="small" @click="payBtn(scope.row)">缴费</el-button>
                    <el-button v-if="hasPerm('wy:rentalfee:edit') && scope.row.payStatus == '0'" type="primary"
                        icon="el-icon-edit" size="small" @click="editBtn(scope.row)">编辑</el-button>
                    <el-button v-if="hasPerm('wy:rentalfee:delete')" type="danger" icon="el-icon-delete" size="small"
...
import { getRentalFeeListApi, getOnHomeList, addRentalFeeApi, editRentalFeeApi, deleteRentalFeeApi, payRentalFeeApi } from "@/api/fee/rental"
...
export default {
    ...
    methods: {
        ...
        async payBtn(row) {
            ...
            if (confrim) {
                let parm = {
                    rentalfeeId: row.rentalfeeId,
                    payStatus: '1'
                }
                let res = await payRentalFeeApi(parm);
                if (res && res.code == 200) {
                    // 删除成功刷新列表
                    this.getListData();
        ...
        async deleteBtn(row) {
            ...
            if (confrim) {
                let res = await deleteRentalFeeApi({ rentalfeeId: row.rentalfeeId });
                if (res && res.code == 200) {
                    // 删除成功刷新列表
                    this.getListData();
                    this.$message.success(res.msg);
            ...
            this.$refs.addForm.validate(async (valid) => {
                if (valid) {
                    ...
                    if (this.addModel.formtype == "0") {
                        // 新增
                        res = await addRentalFeeApi(this.addModel);
                    } else if (this.addModel.formtype == "1") {
                        // 编辑
                        res = await editRentalFeeApi(this.addModel);
                    }
                    if (res && res.code == 200) {
                        // 请求成功刷新列表数据
                        this.getListData();
        ...
        async getListData() {
            let res = await getRentalFeeListApi(this.parms);
            console.log(res);
            if (res && res.code == 200) {
