  ...
  <el-main height="">
    ...
    <el-form :model="parms" ref="searchForm" label-width="80px" :inline="true" size="small">
      <el-form-item label="楼栋名称">
        <el-input v-model="parms.buildName"></el-input>
      </el-form-item>
      <el-form-item label="楼栋类型">
        <el-select v-model="parms.type" placeholder="" clearable filterable style="width: 184px;">
      ...
      <el-form-item>
        ...
        <el-button v-if="hasPerm('wy:building:add')" @click="addBtn" type="primary" icon="el-icon-plus">新增</el-button>
      </el-form-item>
    </el-form>
    <!--
            楼栋列表
    ...
    <el-table :data="tableList" empty-text="暂无数据" :height="tableHeight" size="small" border stripe>
      ...
      <el-table-column align="center" width="100" prop="type" label="楼栋类型">
      ...
      <el-table-column align="center" width="200" label="操作">
        <template slot-scope="scope">
          <el-button v-if="hasPerm('wy:building:edit')" type="primary" icon="el-icon-edit" size="small" @click="editBtn(scope.row)">编辑</el-button>
          <el-button v-if="hasPerm('wy:building:delete')" type="danger" icon="el-icon-delete" size="small" @click="deleteBtn(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--
    ...
    <sys-dialog :title="dialog.title" :visible="dialog.visible" :width="dialog.width" :height="dialog.height"
      ...
      <div slot="content">
        ...
        <el-form :model="addModel" ref="addForm" :rules="rules" label-width="100px" :inline="true" size="small"
          ...
          <el-form-item prop="buildName" label="楼栋名称">
            <el-input v-model="addModel.buildName"></el-input>
          </el-form-item>
          <el-form-item prop="type" label="楼栋类型">
            <el-select v-model="addModel.type" clearable filterable style="width: 184px;">
              <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
...
<script>
import { getBuildingListApi, addBuildingApi, editBuildingApi, deleteBuildingApi } from "@/api/house/building"
...
export default {
  ...
  data() {
    return {
      //表单验证规则
      rules: {
        buildName: [{ trigger: "change", required: true, message: "请填写楼栋名称" }],
  ...
  methods: {
    // 删除按钮事件
    async deleteBtn(row) {
      let confrim = await this.$myconfirm("确定删除该楼栋吗？");
      if (confrim) {
        let res = await deleteBuildingApi({ buildId: row.buildId });
        if (res && res.code == 200) {
          // 删除成功刷新列表
          this.getListData();
    ...
    async getListData() {
      let res = await getBuildingListApi(this.parms);
      console.log(res);
      if (res && res.code == 200) {
