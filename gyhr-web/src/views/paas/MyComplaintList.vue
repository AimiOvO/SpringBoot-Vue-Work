    ...
    <el-main height="">
        ...
        <el-form :model="parms" ref="searchForm" label-width="80px" :inline="true" size="small">
            ...
            <el-form-item>
                ...
                <el-button v-if="hasPerm('wy:mycomplaint:add')" @click="addBtn" type="primary" icon="el-icon-plus">新增</el-button>
            </el-form-item>
        </el-form>
        <!--
            楼栋列表
        ...
        <el-table :data="tableList" @expand-change="expandChange" :expand-row-keys="expands" row-key='complaintId'
            ...
            <el-table-column align="center" width="200" label="操作">
                <template  v-if="scope.row.status == '0'" slot-scope="scope">
                    <el-button v-if="hasPerm('wy:mycomplaint:edit')" type="primary" icon="el-icon-edit" size="small" @click="editBtn(scope.row)">编辑</el-button>
                    <el-button v-if="hasPerm('wy:mycomplaint:delete')" type="danger" icon="el-icon-delete" size="small" @click="deleteBtn(scope.row)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!--
...
<script>
import { getComplaintListApi, addComplaintApi, editComplaintApi, deleteComplaintApi } from "@/api/paas/complaint"
...
export default {
    ...
    methods: {
        ...
        async deleteBtn(row) {
            ...
            if (confrim) {
                let res = await deleteComplaintApi({ complaintId: row.complaintId });
                if (res && res.code == 200) {
                    // 删除成功刷新列表
                    this.getListData();
